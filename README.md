...
| opencode-vim-plugin | Vim controls for the opencode prompt, `~/.config/opencode/tui.json` | Installed by `run_once_after_30-install-opencode-vim-plugin.sh`; requires opencode 1.17.10+ |
...
run_once_after_30-install-opencode-vim-plugin.sh
```
...
`run_once_after_30-install-opencode-vim-plugin.sh` installs
`@leohenon/opencode-vim-plugin` globally (`opencode plugin ... --global`) when
opencode is available. It is skipped on machines without opencode. Plugin
options live in the tracked `~/.config/opencode/tui.json`; the plugin package
itself is cached under `~/.cache/opencode` and is regenerated by the script.
